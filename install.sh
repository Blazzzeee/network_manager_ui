...
echo -e "\033[1;34mMoving network_manager to ~/.config/\033[0m"
mv -r networkmanager/ ~/.config/
if [ $? -eq 0 ]; then
...
# Move network_manager to /usr/local/bin/
echo -e "\033[1;34mMoving network_manager to /usr/local/bin/\033[0m"
sudo mv network_manager /usr/local/bin/
if [ $? -eq 0 ]; then
